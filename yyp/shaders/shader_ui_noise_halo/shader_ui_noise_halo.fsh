///@description https://www.shadertoy.com/view/3tBGRm
///@author magician0809 2019-11-29
...
void main() {
  vec2 uv = (v_vTexcoord*2.-iResolution.xy)/iResolution.y;
  vec4 col;
  draw(col, uv);
  vec3 bg = BG_COLOR;
  vec4 textureColor = texture2D(gm_BaseTexture, v_vTexcoord);
  vec3 pixel = mix(bg, col.rgb, col.a); //normal blend
  gl_FragColor = vec4(
